...
<template>
    <aside class="flex flex-col w-64 h-screen px-4 py-8 overflow-y-auto bg-black border-r rtl:border-r-0 rtl:border-l  dark:bg-black dark:border-gray-700">
    <a :href="route('home')">
    ...
    <div class="flex flex-col justify-between flex-1 mt-6">
        <nav>
            ...
            :href="route('dashboard')"
             v-if="$page.props.auth.user.role === 'admin'|| $page.props.auth.user.role === 'agent'"
            class="flex items-center px-4 py-2 mt-5 text-gray-400 transition-colors duration-300 transform rounded-md dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-gray-200 hover:text-gray-700" >
    ...
    <span class="mx-4 font-medium">Tickets</span>
            ...
            <Link
             class="flex items-center px-4 py-2 mt-5 text-gray-400 transition-colors duration-300 transform rounded-md dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-gray-200 hover:text-gray-700" >
                ...
                <span class="mx-4 font-medium">Changes</span>
            </Link>
            ...
            <button @click="toggleDark()" type="button" class="flex items-center px-4 py-2 mt-5 text-gray-400 transition-colors duration-300 transform rounded-md dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-gray-200 hover:text-gray-700" >
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-circle-half" viewBox="0 0 16 16">
                <path d="M8 15A7 7 0 1 0 8 1zm0 1A8 8 0 1 1 8 0a8 8 0 0 1 0 16"/>
                </svg>
                <span class="mx-4 font-medium">Themes: {{isDark ? 'Dark' : 'Light'}}</span>
            </button>
        ...
        <a href="#" class="flex items-center px-4 -mx-2 mt-5">
      <!---  <img class="object-cover mx-2 rounded-full h-9 w-9" src="https://images.unsplash.com/photo-1531427186611-ecfd6d936c79?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=634&q=80" alt="avatar" />
        -->       
      <div class="bg-gray-400 text-white flex items-center justify-center font-bold rounded-full h-9 w-9 mx-2" >
        {{ $page.props.auth.user.name.charAt(0).toUpperCase() }}
        </div> 

        <span class="mx-2 font-medium text-gray-200 dark:text-gray-200"><Link>{{$page.props.auth.user.name}}</Link></span>
        </a>
    </div>
</aside>
</template>
